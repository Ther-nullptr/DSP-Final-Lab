...
if __name__ == '__main__':
    ...
    df['distance'] = get_distance(df, df[127:128]) * 6371
    df.loc[0:0, 'col4'] += 10
    # sort the df
    ...
    plt.plot(np.diff(df[0:1][temperature_column].to_numpy().squeeze(),n=2) - avg_data,
             label='average diff',
             marker='o')
    plt.plot(exp_avg_data, label='exp_average', marker='o')
    plt.plot(np.diff(df[0:1][temperature_column].to_numpy().squeeze(),n=2) - exp_avg_data,
