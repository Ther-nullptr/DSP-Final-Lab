...
if __name__ == '__main__':
    ...
    df['distance'] = get_distance(df, df[53:54]) * 6371
    ...
    for i in range(point_num):
        ...
        # exp avg
            if i == 1:
                exp_avg_data = data
                exp_sum = np.exp(-df[i:i + 1]['distance'].to_numpy().squeeze() / temp)
            else:
    ...
    print((np.max(np.abs(df[0:1][temperature_column].to_numpy().squeeze() - exp_avg_data))))